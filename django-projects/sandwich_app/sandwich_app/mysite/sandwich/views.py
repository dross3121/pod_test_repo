...
def sandwich_menu(request):
    sandwich_list = []
    for cheese in ingredients['cheeses']:
        for meat in ingredients['meats']:
            for topping in ingredients['topping']:
                sandwich_list.append(f'{cheese}, {meat}, {topping}')
    if request.method == 'GET':
        return render(request=request,
                      template_name='full_menu.html',
                      context={'sandwich': sandwich_list})
