...
# List. Because lists have multiole indeces separated by commas
...
check_out("BORN A CRIME")
# # Removed BORN A CRIME from books list
print()
print()
print()
...
def search_by_name(str):
    ...
    # https://www.kite.com/python/answers/how-to-find-the-index-of-an-item-in-a-list-in-python#:~:text=Use%20the%20in%20keyword%20to,loop%20iterates%20through%20these%20tuples.
    availability = [f"{book_lookup} is available" for (index,item) in enumerate(books) if (item == book_lookup)]
    print("".join(availability))
    if not availability:
        print(f"{book_lookup} is NOT available")
...
# print(2.0)
# # The structure of data is a list of dictionaries. Each dictionary has a key/value pair. A list is a group on indeces seprated by a comma. Each dictionary is separated by a comma
...
#  # for key in books_with_details:
#               # if(books_with_details["authors"] == author_name):
