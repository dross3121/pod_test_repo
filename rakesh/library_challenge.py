...
# It is a 'list' data type because it is enclosed in brackets. I can check it with the 'type' function: print(type(books))
...
# Return: Not needed for this function
def check_out(rm_book):
    books.remove(rm_book)
# 1.4
...
# Parameters: book (string)
# Return: Not needed for this function
def check_in(checkin_book):
    books.append(checkin_book)
...
# books_with_details is a nested data structure with a dictionary inside a list. I know this because the of the "[]" and "{}" around the strings.
# 2.1
...
def search_by_author(author):
    titles_of_books = []
    for i in range(0,len(books_with_details)):
        if books_with_details[i]['author'] == author:
            titles_of_books.append(books_with_details[i]['title'])
    if titles_of_books == []:
        titles_of_books = 'Author Not Found'
    return titles_of_books
